...
//Input from mapper((key=author),value=word totalcount -words per author
//output((key=author & word),value=word totalcount -words per author,max, tf
//First Job
...
public class TFIDReducer extends Reducer<Text,Text,Text,Text> {
	...
	public void reduce(Text key,Iterable<Text> values,Context context)throws IOException,InterruptedException
	{
		...
		for (String wordasKey : hashset.keySet()) {
            ...
            context.write(new Text(key.toString()), new Text(wordasKey+"\t"+hashset.get(wordasKey)+ "\t"
                    + max+"\t"+ (float)hashset.get(wordasKey)/max));
        }
