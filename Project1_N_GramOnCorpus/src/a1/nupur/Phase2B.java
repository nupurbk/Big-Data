...
public class Phase2B {
				...
				if (read_start) {
					...
					while (tokenizer.hasMoreTokens()) {
						...
						if(word.length()!=0)
						{
							...
							double count = 0;
							if (hm.containsKey(bigram+ ":" + author)) {
								count = hm.get(bigram+ ":" + author);
							}
							start = word;
							hm.put(bigram + ":" + author, count + 1);
						}

					}
					String bigram2 = start + "," + end;
					if(hm.containsKey(bigram2 + ":" + author)) {
						endCount = hm.get(bigram2 + ":" + author);

					}
					hm.put(bigram2 + ":" + author, endCount + 1);
	...
	public static void main(String[] args) throws Exception {
		...
		Job job = new Job();
		job.setJarByClass(Phase2B.class);
		job.setJobName("Phase2B");
